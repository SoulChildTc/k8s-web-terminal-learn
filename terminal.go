...
func startProcess(c *gin.Context, cmd []string, ptyHandler PtyHandler) error {
...
func WaitForTerminal(c *gin.Context, sessionId string) {
	...
	case <-terminalSessions.Get(sessionId).bound:
		...
		if isValidShell(validShells, shell) {
			cmd := []string{shell}
			err = startProcess(c, cmd, terminalSessions.Get(sessionId))
		} else {
			// No shell given or it was not valid: try some shells until one succeeds or all fail
			// FIXME: if the first shell fails then the first keyboard event is lost
			for _, testShell := range validShells {
				cmd := []string{testShell}
				if err = startProcess(c, cmd, terminalSessions.Get(sessionId)); err == nil {
					break
				}
			}
		}
